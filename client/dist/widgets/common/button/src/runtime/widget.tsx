...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  showQuickStylePanel = (): boolean => {
    ...
    return showQuickStyle && active && repeat !== RepeatType.Sub
  }
  ...
  getLinkComponent = () => {
    ...
    let accessibilityLabel = this.state.toolTip || text || config.functionConfig?.icon?.data?.properties?.filename
    ...
    const leftIcon = icon && (!icon.position || icon.position === IconPosition.Left) && <Icon icon={icon.data.svg} className={classNames({ 'mr-2 ml-0': !!text && !isRTL, 'ml-2 mr-0': !!text && isRTL, 'mx-0': !text })}/>
    const rightIcon = icon && icon.position && icon.position === IconPosition.Right && <Icon icon={icon.data.svg} className={classNames({ 'ml-2 mr-0': !!text && !isRTL, 'mr-2 ml-0': !!text && isRTL, 'mx-0': !text })}/>
    ...
    return <Link to={linkTo} target={target} queryObject={queryObject}
      title={accessibilityLabel} className={basicClassNames} role={linkTo?.value && linkTo.linkType !== LinkType.Dialog ? 'link' : 'button'}

import { type DataSource, DataSourceManager, DataSourceStatus, type QueriableDataSource, MessageManager, DataRecordSetChangeMessage, RecordSetChangeType, urlUtils, utils, Immutable } from 'jimu-core'
import {
  RECENT_SEARCHES_KEY, type IMConfig, type SearchDataConfig, type SuggestionItem, type ServiceList,
...
import { fetchLayerSuggestion, loadDsRecords, updateAllMainDsQueryParams } from './search-service'
...
export function updateAllLayerServiceDsQueryParams (option: UpdateQueryParamsOption) {
  ...
  for (const configId in serviceList) {
    ...
    if (searchServiceType === SearchServiceType.FeatureService && useDataSource?.dataSourceId) {
      ...
      let newSqlList = datasourceSqlList?.[dsId] || null
      ...
      datasourceSqlList[dsId] = newSqlList
    }
  }
...
export const clearFilterOfDeletedDs = (serviceList: IMDatasourceListItem, widgetId: string, configId: string, enableFiltering: boolean) => {
  ...
  if (enableFiltering && useDataSource) {
    (useDataSource as QueriableDataSource).updateQueryParams({}, localId)
  }
}

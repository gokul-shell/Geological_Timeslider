...
import {
  ...
  publishRecordCreatedMessageAction, updateAllLayerServiceDsQueryParams, updateRecordsOfOutputDs, checkIsAllRecordLoaded
} from '../utils/utils'
...
const SearchInput = (props: SearchSettingProps) => {
  ...
  useEffect(() => {
    clearDsFilterAfterDeleteDs(serviceList)
  ...
  const clearDsFilterAfterDeleteDs = (serviceList: IMServiceList) => {
    if (!didMount.current) return false
    ...
    const isHasDeleteService = deleteConfigIds?.length > 0

    if (isHasDeleteService) {
      clearTimeout(clearSearchValueTimeoutRef.current)
      clearSearchValueTimeoutRef.current = setTimeout(() => {
        clearSearchValue()
        clearSearchResult()
  ...
  */
  ...
  const initResultServiceList = (newServiceList: ServiceList, initResultServiceListOption?: InitResultServiceListOption) => {
    ...
    if (magicKey || isFromSuggestion) {
      newResultServiceList = Immutable(suggestionServiceList)
    }
  ...
  */
  const setQuerySQL = useCallback((searchText: string, initResultServiceListOption?: InitResultServiceListOption) => {

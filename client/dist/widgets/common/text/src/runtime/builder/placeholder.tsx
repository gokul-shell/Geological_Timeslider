...
import { type RichTextEditorProps, type Editor, type Sources, RichTextEditor, Delta, type DeltaValue, richTextEditorUtils } from 'jimu-ui/advanced/rich-text-editor'
...
const normolizeLineSpace = (_, delta: DeltaValue) => {
  delta.forEach((op) => {
    // Only numeric line-height is supported
    if (typeof op?.attributes?.linespace === 'string') {
...
const modules = {
  ...
  clipboard: {
    matchers: [
      ['img', () => new Delta()],
      ['p', normolizeLineSpace],
      ['li', normolizeLineSpace]
    ]
  }
}

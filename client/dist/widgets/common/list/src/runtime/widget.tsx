...
export class Widget extends React.PureComponent<ListProps, ListStates> {
        ...
        <div className="d-flex align-items-center">
          ...
          {(ds && isShowDataAction) && <div className={classNames('list-data-action position-relative', { 'm-left': listWidth < LIST_TOOL_MIN_SIZE })} data-testid="data-action">
            <DataActionList
              ...
              dataSets={[{ dataSource: ds, records: selectRecords, name: dataName, type: 'selected' }]}
  ...
  render () {
    const { config, id, appMode, browserSizeMode, queryObject, selectionIsInSelf, selectionIsSelf, useDataSources, builderStatus, layouts, activeSort } = this.props
    const { forceShowMask, datasource, widgetRect, page, createDataSourceFailed, sortOptionName, searchText, filterApplied, currentFilter } = this.state
    const appConfig = getAppStore().getState().appConfig

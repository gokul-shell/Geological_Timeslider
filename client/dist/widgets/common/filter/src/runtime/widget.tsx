/** @jsx jsx */
import {
  React, classNames, css, type AllWidgetProps, jsx, type DataSource, type QueriableDataSource,
  dataSourceUtils, type ImmutableArray, type UseDataSource, type ImmutableObject, Immutable, type IMSqlExpression,
  type IconResult, DataSourceStatus, MessageManager, DataSourceFilterChangeMessage, lodash
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<WidgetConfig>, State> {
  ...
  setSqlToAllDs = () => {
    Object.keys(this.state.dataSources).forEach(dsId => {
      ...
      if (ds) { // exclude invalid dataSources
        const sqlResult = this.getQuerySqlFromDs(ds)
  ...
  onFilterItemChange = (index: number, dataSource: DataSource, sqlExprObj: IMSqlExpression, applied: boolean) => {
    ...
    if (!this.state.filterItems[index] && applied) {
      this.setSqlToDs(dataSource, { sql: '', sqlExpression: null })
      return
    }
    ...
    if (needQuery) {
      const sqlResult = this.getQuerySqlFromDs(dataSource, filterItems)
      this.setSqlToDs(dataSource, sqlResult)
    }
  }

  setSqlToDs = (dataSource: DataSource, sqlResult) => {
    // empty SQL set for ds would cause more useless requests.
  ...
  getQuerySqlFromDs = (dataSource: DataSource, filterItems = this.props.config.filterItems) => {
    ...
    filterItems.forEach(item => {
      ...
      if (item.useDataSource.dataSourceId === dataSource?.id && (item.autoApplyWhenWidgetOpen || (this.props.config.omitInternalStyle && getShownClauseNumberByExpression(item.sqlExprObj) === 1))) {
        const sqlResult = dataSourceUtils.getArcGISSQL(item.sqlExprObj, dataSource)
        if (sqlResult.sql) {

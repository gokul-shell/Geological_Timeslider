...
import {
  ...
  type IMIconResult, ClauseLogic, urlUtils, defaultMessages as jimuCoreMessages, type DataSource, getAppStore, APP_FRAME_NAME_IN_BUILDER
} from 'jimu-core'
...
interface State {
  ...
  defaultFilterIconResult: IMIconResult
}
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<WidgetConfig> & State, any> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      dataSources: {},
      filterIconResult: DefaultIconResult.setIn(['properties', 'filename'], this.i18nMessage('filter'))
    }
  }
  ...
  dataSourceChangeForFI = (useDataSources: UseDataSource[]) => {
    ...
    this.dsManager.createDataSourceByUseDataSource(Immutable(useDataSources[0])).then(currentDs => {
      const filterItem: filterItemConfig = {
        icon: this.state.filterIconResult,
      ...
      <div className='jimu-widget-setting widget-setting-filter h-100' css={getStyleForWidget(this.props.theme)}>
        ...
        >
          <FilterItem
            ...
            customIcons={[this.state.filterIconResult.asMutable({ deep: true })]}

...
> {
  ...
  componentDidUpdate (prevProps: AllWidgetProps<IMConfig> & Props) {
    const { id, config } = this.props
    const { editFeatures, dataSources, activeId, jimuMapView } = this.state
    const { editMode, layersConfig, selfSnapping, featureSnapping, defaultSelfEnabled, defaultFeatureEnabled, defaultSnapLayers } = config
    const { config: preConfig } = prevProps
    const {
      editMode: preEditMode, layersConfig: preLayersConfig, selfSnapping: preSelf, featureSnapping: preFeature,
      defaultSelfEnabled: preDefaultSelfEnabled, defaultFeatureEnabled: preDefaultFeatureEnabled, defaultSnapLayers: preDefaultSnapLayers
    } = preConfig
    ...
    const snapChange = (selfSnapping !== preSelf) || (featureSnapping !== preFeature) || (defaultSelfEnabled !== preDefaultSelfEnabled) ||
      (defaultFeatureEnabled !== preDefaultFeatureEnabled) || (defaultSnapLayers !== preDefaultSnapLayers)
    if (snapChange) {
      const snapOn = selfSnapping || featureSnapping
      const defaultSnapSources = this.getDefaultSnapSources(jimuMapView, defaultSnapLayers)
  ...
  renderGroup = (group) => {
    ...
    return (
      <div role='group' aria-label={group.label} className={CSS.group} key={group.label}>
  ...
  editorLayerWatcher = (event, id: string, layerName: string) => {
    const { dataSources, editFeatures } = this.state
    ...
    if (updates) {
      changedType = SparkChangedType.Updated
      const updateFeature = event?.edits?.updateFeatures?.[0]
      const originalAttributes = (this.edit.viewModel as __esri.EditorViewModel)?.featureFormViewModel?.feature?.attributes || {}
      ...
      if (updateFeature) {
        ...
        dataSource.afterUpdateRecord(record)
        this.startWorkFlowWhenAwait(editFeatures)
      }
    }
    if (deletes) {
      changedType = SparkChangedType.Deleted
      ...
      if (editorUseLayer) {
        if (activeConfigLayer) { // It has been configured in setting
          ...
          watchEditLayer.on('edits', event => {
            this.editorLayerWatcher(event, id, watchEditLayer.title)
          })
        } else {
          editLayerInfos.push({
            layer: editorUseLayer,
  ...
  createEditor = (mapView?: JimuMapView) => {
    ...
    const { id, config } = this.props
    const { layersConfig, selfSnapping, featureSnapping, defaultSelfEnabled, defaultFeatureEnabled, defaultSnapLayers } = config
    ...
    const newEditor = () => {
      ...
      const that = this
      reactiveUtils.watch(() => this.edit.viewModel.state, editState => {
  ...
  onDataSourceSelectedChange = async (dataSourceId: string, selectedIds: string[]) => {
    ...
    const editCount = flatEditFeatures.length
    const step = editCount > 1 ? 'list' : editCount === 1 ? 'form' : 'empty'
    if (editMode === EditModeType.Geometry && editCount === 0) {
      const editor = this.edit as Editor
    ...
    //
    if (editMode === EditModeType.Geometry && selectedRecords?.length === 0) {
      const editor = this.edit as Editor
      if (editor?.activeWorkflow) editor?.cancelWorkflow()
    }
    ...
    if (editMode === EditModeType.Geometry) {
      if (!jimuMapView) return
      this.setState({ editFeatures: newEditFeatures, activeId: dataSourceId, selectionStartWorkflow: true }, () => {
  ...
  render () {
    const { activeId, editFeatures, featureFormStep, formEditable, delConfirm, attrUpdating, formChange, formSubmittable, loading } = this.state

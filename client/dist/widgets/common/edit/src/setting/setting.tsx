...
> {
  ...
  onFilterDs = (dsJson: IMDataSourceJson): boolean => {
    ...
    const getOperRights = (dsId: string): boolean => {
      ...
      const layerEditingEnabled = (dataSource as any)?.layer?.editingEnabled ?? true
      ...
      const deletable = this.getDsCap(capabilities, 'delete')
      return layerEditingEnabled && (create || editing || deletable)
    }
  ...
  importAll = () => {
    ...
    allLayers.forEach(layer => {
      ...
      const layerEditingEnabled = (layer?.getMainDataSource() as any)?.layer?.editingEnabled ?? true
      if (haveUrl && layerEditingEnabled && layersConfig.findIndex(config => config.id === layer.id) === -1) {
        useDataSources.push({
  ...
  getAllLayersOptions = (): MultiSelectItem[] => {
    const res = []
    const { useMapWidgetIds } = this.props
    const useMapWidget = useMapWidgetIds?.[0]
    const allLayers = getMapAllLayers(useMapWidget)
    if (allLayers.length > 0) {
      allLayers.forEach(item => {
        const dsJson = item.getDataSourceJson()
        res.push({
          value: dsJson.id,
  ...
  handleSnapLayersChange = (evt, value: string, values: string[]) => {
    this.onPropertyChange('defaultSnapLayers', values)
  }
  ...
  render () {
    ...
    const { layersConfig, editMode, selfSnapping, featureSnapping, defaultSelfEnabled, defaultFeatureEnabled, defaultSnapLayers } = config
              ...
              {isGeoMode &&
                ...
                >
                  ...
                  >
                    <MultiSelect
                      size='sm'
                      items={this.getAllLayersOptions()}
                      values={defaultSnapLayers}
                      className='search-multi-select'
                      fluid
                      onClickItem={this.handleSnapLayersChange}
                      displayByValues={this.displaySelectedLayers}

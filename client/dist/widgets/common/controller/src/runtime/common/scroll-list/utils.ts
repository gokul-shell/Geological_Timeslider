...
const useObserveDebouncedLength = (
  ...
  enable?: boolean) => {
  let length = _useObserveDebouncedLength(rootRef, 500, vertical, enable)
  length += offset
...
export const useResponsiveViewport = (props: ResponsiveViewportProps): ResponsiveViewportResult => {
  ...
  const autoSizeRef = hooks.useLatest(autoSize)
  ...
  const number = React.useMemo(() => {
    if (autoSizeRef.current) {
      ...
      return Math.floor(length / itemLength)
    }
  }, [autoSizeRef, counts, itemLength, length])

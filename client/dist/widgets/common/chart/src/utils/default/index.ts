...
import { type HistogramOverlaysType, type WebChartSeries } from '../../config'
...
export const getNonRepeatingSeriesFillColor = (index: number = 0, preSerieColor: string): string => {
  let color = getSeriesFillColor(index)
  if (color === preSerieColor) {
    color = getSeriesFillColor(index + 1)
  }
  return color
}
 ...
 */
export function getDefaultBarChartSeries (index: number = 0, preSerieColor?: string): WebChartSeries {
  const color = getNonRepeatingSeriesFillColor(index, preSerieColor)
  return {
 ...
 */
export function getDefaultLineChartSeries (index: number = 0, preSerieColor?: string): WebChartSeries {
  const color = getNonRepeatingSeriesFillColor(index, preSerieColor)
  return {

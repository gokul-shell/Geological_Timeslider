/**@jsx jsx */
import { React, jsx, css, classNames, JimuFieldType, hooks } from 'jimu-core'
...
export const ColorAdder = (props: ColorAdderProps): React.ReactElement => {
  const { className, useFeatureLayerDataSource = false, categoryFieldType, validity, onChange } = props
  ...
  const handlekeyDown = (evt: React.KeyboardEvent<HTMLInputElement>) => {
    if (evt.key === 'Enter') {
      if (!useFeatureLayerDataSource && categoryFieldType === JimuFieldType.Number) {
    ...
    <div className={classNames('color-adder', className)} css={style}>
      ...
      {editable && <div className='editor' role='group' aria-label={translate('addCategory')}>
        <div className='top-part'>
          ...
          <Button aria-label={translate('commonModalCancel')} icon type='tertiary' size='sm' onClick={handleCancleClick} title={translate('commonModalCancel')}><MinusCircleOutlined size='m' /></Button>
        </div>

      ...
      {propAxes?.map((axis, index) => {
        ...
        return (
          ...
          >
            ...
            {
              type === 'number' && (
                <NumericAxis className='mt-3' showValueRange={index === 0 ? showFirstValueRange : true} showIntegerOnly={index === 0 ? showFirstIntegerOnly : true} showLogarithmicScale={showLogarithmicScale} isHorizontal={isHorizontal} axis={axis} onChange={handleChange} />
              )
            }
            {
              type === 'date' && (

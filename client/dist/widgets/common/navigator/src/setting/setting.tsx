...
import { React, css, jsx, Immutable, polished, type IMThemeSliderVariant, type IMThemeVariables, hooks } from 'jimu-core'
...
import { type NavigationVariant, MultiSelect, Select, Radio, Label, Switch, TextInput, NumericInput, Tooltip, Button, defaultMessages as jimuiDefaultMessage } from 'jimu-ui'

import { getSectionLabel, useViewNavigationVariant, getViewNavigationVariant, getViewSelectItems, useContainerSections, useSectionViews, useIconNLSNames } from './utils'
...
import { type ComponentState, NavStyleSettingByState, NavIconPicker, InputUnit, TextAlignment } from 'jimu-ui/advanced/style-setting-components'
import defaultMessages from './translations/default'
import { useTheme2 } from 'jimu-theme'
import { InfoOutlined } from 'jimu-icons/outlined/suggested/info'
import { setWidgetSize, useCustomIcons } from '../utils'
...
const Setting = (props: SettingProps) => {
  ...
  const { vertical, type, navStyle, variant: cfVariant, advanced, standard, paginationFontColor } = display
  const { showIcon, showText, showTitle, previousText, nextText, hideThumb, step = 1, textAlign, gap, showPageNumber } = standard || {}
  const variant = useViewNavigationVariant(type, navStyle, advanced, cfVariant, standard)
  ...
  const isTabStyle = React.useMemo(() => type === 'nav' && showText, [showText, type])

  const isSymbolStyle = React.useMemo(() => type === 'nav' && !showText, [showText, type])
  ...
  const onAdvancedChange = () => {
    ...
    if (advanced) {
      const variant = getViewNavigationVariant(type, navStyle, showText)
  ...
  const viewSelectItemLabels = React.useMemo(() => viewSelectItems.asMutable().map((item) => item.label), [viewSelectItems])
  ...
  return <div className="widget-setting-navigator jimu-widget-setting" css={style}>
    <SettingSection>
      ...
      {section && <React.Fragment>
        ...
        {viewType === ViewType.Custom && <SettingRow flow="wrap">
          <MultiSelect aria-label={viewSelectItemLabels} values={cfView} items={viewSelectItems} onClickItem={onViewsSelectClick} displayByValues={renderSelectText} ></MultiSelect>
        </SettingRow>}

        {type !== 'slider' && <SettingRow flow="no-wrap" label={translate('direction')} aria-label={translate('direction')} role='group'>
        ...
        {isNavType && <SettingRow flow="no-wrap" label={translate('alignment')} aria-label={translate('alignment')} role='group'>
          <TextAlignment textAlign={textAlign} onChange={(value) => { onSettingConfigChange(['display', 'standard', 'textAlign'], value) }} />
        </SettingRow>}

        {isTabStyle && <SettingRow flow='no-wrap' label={translate('showIcon')}>
          <Switch
        ...
        {
          isSymbolStyle && <SettingRow flow="no-wrap" label={translate('tooltip')} aria-label={translate('tooltip')}>
        ...
        {isSymbolStyle && <React.Fragment>
          ...
          <SettingRow flow="no-wrap" label={translate('currentView')} aria-label={translate('currentView')}>
            <NavIconPicker configurableOption={'none'} hideRemove size={8} icon={activedIconWithNLSName as any} customLabel={activedIconWithNLSName?.properties?.filename} onChange={(icon) => { onSettingConfigChange(['display', 'standard', 'activedIcon'], icon) }}></NavIconPicker>
          </SettingRow>
          <SettingRow flow="no-wrap" label={translate('others')} aria-label={translate('others')}>
            <NavIconPicker configurableOption={'none'} hideRemove size={8} icon={alternateIconWithNLSName as any} customLabel={alternateIconWithNLSName?.properties?.filename} onChange={(icon) => { onSettingConfigChange(['display', 'standard', 'alternateIcon'], icon) }}></NavIconPicker>
          </SettingRow>
        </React.Fragment>
        }
...

        {
          type === 'navButtonGroup' && <React.Fragment>
            <SettingRow flow="wrap" label={translate('previous')} role='group' aria-label={translate('previous')} className="justify-content-between color-drak-600">
              ...
              <IconPicker className='nav-button-icon-picker' configurableOption={'none'} icon={previousIconWithNLSName as any} customIcons={customIcons} customLabel={previousIconWithNLSName?.properties?.filename} onChange={(icon) => { onSettingConfigChange(['display', 'standard', 'previousIcon'], icon) }} setButtonUseColor={false}></IconPicker>
            </SettingRow>
            ...
            <SettingRow flow="wrap" label={translate('next')} role='group' aria-label={translate('next')} className="justify-content-between color-drak-600">
              ...
              <IconPicker className='nav-button-icon-picker' configurableOption={'none'} icon={nextIconWithNLSName as any} customIcons={customIcons} customLabel={nextIconWithNLSName?.properties?.filename} onChange={(icon) => { onSettingConfigChange(['display', 'standard', 'nextIcon'], icon) }} setButtonUseColor={false}></IconPicker>
            </SettingRow>
    ...
    {section && <SettingSection>
      <SettingRow flow="no-wrap" label={translate('advance')} aria-label={translate('advance')} role='group' className='color-drak-600'>

...
export const getViewNavigationVariant = (type: ViewNavigationType, navStyle: string, showText: boolean): ViewNavigationVariant => {
  ...
  const iconSize = showText ? '16px' : '8px'
...
export const useViewNavigationVariant = (type: ViewNavigationType, navStyle: string, advanced: boolean, advanceVariant: ViewNavigationVariant, standard: Immutable.ImmutableObject<ViewNavigationStandard>): ViewNavigationVariant => {
  return useMemo(() => {
    ...
    return getViewNavigationVariant(type, navStyle, standard?.showText)
  }, [advanced, advanceVariant, type, navStyle, standard?.showText])
}

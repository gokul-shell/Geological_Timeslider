...
import { type IMConfig, type IMPrintTemplateProperties, type PrintTemplateProperties, PrintExtentType, WKID_LINK, Views } from '../../../config'
...
const TemplateSetting = (props: Props) => {
  ...
  `
  const { templateList, jimuMapView, selectedTemplate, id, config, confirmPrint, views, handleSelectedTemplateChange, setSelectedTemplateByIndex } = props
  ...
  useEffect(() => {
    ...
    setLegendEnabled(!!selectedTemplate?.layoutOptions?.legendLayers || selectedTemplate?.layoutOptions?.legendLayers?.length > 0)
    setIncludeNorthArrow(selectedTemplate?.layoutOptions?.elementOverrides?.[northArrowKey]?.visible)

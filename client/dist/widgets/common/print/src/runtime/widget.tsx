...
import { type IMConfig, ModeType, type PrintTemplateProperties } from '../config'
...
const Widget = (props: PrintProps) => {
  const { id, config, dispatch, useMapWidgetIds, layoutId, layoutItemId, locale } = props
    ...
    <div className={classNames('w-100 h-100', { 'surface-1': config?.modeType === ModeType.Classic })} css={STYLE}>
...

      {
        config?.modeType === ModeType.Classic && <div className='w-100 h-100'>
          {!checkShowPlaceholder() && <Classic id={id} locale={locale} config={config} jimuMapView={jimuMapView} templateList={templateList} />}
...

      {
        config?.modeType === ModeType.Compact && <div className='w-100 h-100'>
          <CompactPrint id={id} config={config} locale={locale} jimuMapView={jimuMapView} templateList={templateList} errorTip={getErrorTip()} />
        </div>
      }
    </div>
  )

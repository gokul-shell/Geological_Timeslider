...
import { type IMConfig, DEFAULT_OUTLINE } from '../../config'
...
const PreviewStyle = (props: Props) => {
  ...
  const getLabel = () => {
    return (
      <div className='d-flex align-items-center' onClick={handleEnablePreviewChange}>
        <div className='flex-grow-1 enable-preview-label text-truncate w-100' title={nls('enablePrintPreview')}>{nls('enablePrintPreview')}</div>
        <Switch
          aria-label={nls('enablePrintPreview')}
          checked={config?.enablePreview}
          onChange={handleEnablePreviewChange}
          title={nls('enablePrintPreview')}
        />
      </div>
    )
  }
  ...
  return (
    <SettingSection css={STYLE} role='group' title={getLabel()} aria-label={nls('printPreviewStyle')}>

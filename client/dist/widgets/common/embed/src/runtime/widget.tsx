...
interface State {
  ...
  // Indicates the code is not socail media or video
  useSrcdoc?: boolean
}
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State> {
  ...
  reloadContentInView = (preSectionNavInfos) => {
    ...
    if (
      ...
      if (layoutJson?.parent?.type !== LayoutParentType.View) return
      const viewId = layoutJson?.parent?.id
      const viewJson = appConfig?.views?.[viewId]
      const sectionId = viewJson?.parent
    ...
    if (this.ifr) {
      ...
      if (embedType === EmbedType.Code) {
        if (useSrcdoc) {
          ...
          setTimeout(() => {
            if (this.ifr) this.ifr.src = content
          }, 100)
        }
      } else {
        setTimeout(() => {

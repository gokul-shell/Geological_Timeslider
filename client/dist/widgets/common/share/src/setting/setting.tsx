/** @jsx jsx */
import { React, jsx, Immutable, type ImmutableArray, defaultMessages } from 'jimu-core'
...
import { type IMConfig, UiMode, type Item, InlineDirection, BtnIconSize, IconColorMode } from '../config'
      ...
      <React.Fragment>
        ...
        <SettingSection title={design} aria-label={design} role='group'>
          <SettingRow label={direction}>
            <ButtonGroup>
              <DirectionSelector vertical={isVertical} onChange={onInlineDirChange} />
            </ButtonGroup>
          </SettingRow>
        ...
        <SettingRow label={'Icon style'}>
          <RadiusSelector radius={rad0} btnRad={btnRad} themeVal={theme} onClick={() => onIconStyleChange(rad0)} className="pr-3"></RadiusSelector>
          <RadiusSelector radius={rad1} btnRad={btnRad} themeVal={theme} onClick={() => onIconStyleChange(rad1)} className="pr-3"></RadiusSelector>
          <RadiusSelector radius={rad2} btnRad={btnRad} themeVal={theme} onClick={() => onIconStyleChange(rad2)}></RadiusSelector>
        </SettingRow>
        */}
          <SettingRow label={showMedia}>
            <Switch checked={!props.config.inline.design.hideLabel} onChange={onHideLabelChange}
              aria-label={showMedia} />
          </SettingRow>

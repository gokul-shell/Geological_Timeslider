...
import { ContentServiceWrapper, LayoutServiceProvider, widgetService, type AppConfigAction } from 'jimu-for-builder'
...
> {
  ...
  componentDidUpdate (prveProps) {
    ...
    const isPreviewIdChange =
      prveCardConfigs?.transitionInfo?.previewId ===
      cardConfigs?.transitionInfo?.previewId
    const isStatusChange =
      prveProps.builderStatus &&
      prveProps.builderStatus !== builderStatus &&
      !this.isUpdateFirst
    const isHoverEnableChange =
      prveCardConfigs[Status.Hover].enable !== cardConfigs[Status.Hover].enable
  ...
  handleCopyTo = (evt, status: Status, selectedLayoutItem, linked: boolean) => {
    ...
    if (linked) {
      ...
      const service = LayoutServiceProvider.getService(appConfig, desLayoutId)
      ...
      // add widget parent
      appConfig = widgetService.addParent(appConfig, selectedLayoutItem.widgetId, destLayoutInfo, browserSizeMode)
  ...
  handleBreakLink = evt => {
    ...
    const contentService = new ContentServiceWrapper(action.appConfig, layoutItem)
    ...
    action = builderSupportModules.jimuForBuilderLib.getAppConfigAction(contentService.getConfig())
  ...
  renderCardEditor = () => {
    ...
    const regularBgStyle = cardConfigs[Status.Default].backgroundStyle.setIn(
      ['boxShadow', 'color'],
      'transparent'
    )
    const hoverBgStyle = cardConfigs[Status.Hover].backgroundStyle.setIn(
      ['boxShadow', 'color'],
      'transparent'
    )

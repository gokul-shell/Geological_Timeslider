/** @jsx jsx */
import { React, jsx, css, getAppStore, Immutable, type ImmutableArray, proxyUtils, SupportedUtilityType, type UseUtility } from 'jimu-core'
...
import { Radio, TextArea, Select, Switch, Label, Button, Icon, Checkbox, Popper, Tooltip, NumericInput } from 'jimu-ui'
...
// import { SearchType } from 'jimu-for-builder/service'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<any>, any> {
  ...
  setDefaults () {
    ...
    if (ACLUtils.notDef(this.props.config.runReportOnClick)) {
      changeArr.push({ name: 'runReportOnClick', value: true })
    }
  ...
  reportSelectedHandler (ev: any) {
    ...
    if (widgetMode === Mode.Preset) {
      ...
      // this.onPropertyChange('presetSelectedReport', ev.detail.id)
      // this.onPropertyChange('presetSelectedReportName', ev.detail.name)
    } else {
      ...
      // this.onPropertyChange('workflowSelectedReport', ev.detail.id)
      // this.onPropertyChange('workflowSelectedReportName', ev.detail.name)
    }
    this.onMultiplePropertyChange(changeArr)
  }
  ...
  reportCheckedHandler (ev: any) {
    console.log('%c ExB Setting report changed', 'color:orange;font-size:11pt')
    ...
    this.onPropertyChange('reportList', {
  ...
  componentDidMount () {
    ...
    if (this.props.token) {
      countriesUrl = countriesUrl + '&token=' + this.props.token
    }
    ...
    fetch(countriesUrl)
      ...
      .then(
        (data: any) => {
          self.updateState('countries', data.countries)
          if (typeof self.props.config.availableGeographyLevels === 'undefined') {
            const countryObj = data.countries.find(o => o.id === 'US')
            if (countryObj.hierarchies) {
              for (let i = 0; i < countryObj.hierarchies.length; i++) {
                if (countryObj.hierarchies[i].default) {
                  const availableGeographyLevels = countryObj.hierarchies[i].levelsInfo.geographyLevels
                  self.onPropertyChange('availableGeographyLevels', availableGeographyLevels)
                  self.onPropertyChange('selectedGeographyLevels', availableGeographyLevels)
                }
              }
            }
          }
  ...
  updateSelectedCountry = (country) => {
    ...
    requestAnimationFrame(() => {
      ...
      //self.updateState('selectedCountry', country)
  ...
  updateGeographyLevels = (level, checked) => {
    ...
    // update search component levels
    // document.getElementById(this.props.id + '_' + 'search').setActiveLevels(selectedLevels)
  }
  ...
  onDefaultInfographicChanged (info) {
    if (info) {
      console.log('DEFAULT INFOGRAPHIC CHANGED', info)
    }
  }
  ...
  initializeReportComponents () {
    ...
    requestAnimationFrame(() => {
      ...
      const { sourceCountry, reportList } = self.props.config
      ...
      if (presetReports) {
        presetReports.initialize(user.username, token, sourceCountry, langCode, false, JSON.stringify(reportList), false)
        presetReports.style.width = '100%'
        presetReports.style.padding = '3px !important'
        presetReports.setShowDefault(false)
      }
      const wfReports: any = document.getElementById(self.props.id + '_' + 'wf-reports')
      if (wfReports) {
        if (workflowEnableInfographicChoice) {
          wfReports.initialize(user.username, token, sourceCountry, langCode, true, JSON.stringify(reportList), true)
          wfReports.style.width = '100%'
          wfReports.style.padding = '3px !important'
          wfReports.setShowDefault(true)
          wfReports.setAttribute('showCheckboxes', true)
          wfReports.onDefaultReportChange(self.onDefaultInfographicChanged, this)
        } else {
          wfReports.initialize(user.username, token, sourceCountry, langCode, false, JSON.stringify(reportList), false)
          wfReports.style.width = '100%'
          wfReports.style.padding = '3px !important'
          wfReports.setShowDefault(false)
  ...
  render () {
    const { selectedCountry, activeGeographyLevels, geographiesChecked, pointsOfInterestChecked, presetBuffersQueued, maxDriveBuffer, maxWalkBuffer, stViewMode, portalHelpUrl } = this.state
    const { reportList, availableGeographyLevels, selectedGeographyLevels, drawPointEnabled, drawPolygonEnabled } = this.props.config
    const { widgetMode, igBackgroundColor, runReportOnClick, imageExport, dynamicHtml, excel, pdf, fullscreen, zoomLevel, displayHeader, headerColor, headerTextColor, sourceCountry } = this.props.config
    ...
    return (
      <div css={getStyle(theme)}>
        <div className='widget-setting-bao' style={{ display: 'relative' }}>
              ...
              ? <React.Fragment>
                ...
                {presetSearchSidePopper && (
                  <SidePopper isOpen title={this.localeString('setLocation')} position='right' toggle={() => { this.toggleSidePopper('presetSearchSidePopper') }} trigger={this.sidePopperTrigger?.current}>
                    <div className='p-3'>
                      <SettingRow className='mt-3' label={this.localeString('presetLocationLabel')} />
                      <SettingRow>
                        {presetSearchObj && !presetShowSearchInput
                          ? <Button className='w-100 d-flex selectedStateButton' style={{ flex: '1 1 auto', alignItems: 'stretch' }} onClick={() => { this.clearSearchObj() }} >
                            ...
                            <Tooltip style={{ margin: '10px' }} placement='bottom-start' title={this.localeString('clearSearch')}>
                              <div className='d-flex' style={{ margin: 'auto 0', alignItems: 'right' }}>
                    ...
                    <div className='p-3'>
                      <SettingRow label={
                        <React.Fragment>
                          ...
                          <Button type='tertiary' className='widget-help-btn' icon size='sm' onClick={() => { this.updateState('allowSearchInfoIconOpen', !allowSearchInfoIconOpen) }} ref={this.allowSearchInfoRef as React.RefObject<HTMLButtonElement>} >
                            <InfoOutlined />
                      ...
                      {!workflowEnableSearch && (
                        <React.Fragment>
                          <SettingRow className='mt-3' label={this.localeString('customExtentBoundary')} />
                          <SettingRow>
                      ...
                      {workflowEnableSearch && widgetMode === Mode.Workflow && (
                        <React.Fragment>
                          <SettingRow className="pt-3" label={this.localeString('searchControl')} />
                          <SettingRow>
                            <Label check centric>
                ...
                ? <SettingSection title={<div className='w-100 d-flex' style={{ height: '23px', flexFlow: 'column wrap', alignContent: 'space-between' }}>
                  ...
                  {presetBufferSidePopper && (
                    <SidePopper isOpen title={this.localeString('setBuffers')} position='right' toggle={() => { this.toggleSidePopper('presetBufferSidePopper') }} trigger={this.sidePopperTrigger?.current}>
                      <div className="p-3" style={popperOuterStyle}>
                        <SettingRow>
                          <Label centric>
                        ...
                        {presetBuffersQueued && (
                          <div css={style} className='mx-2 mt-3'>
                            <SettingRow flow='wrap'>
                      ...
                      <div className="p-3" style={popperOuterStyle}>
                        {widgetMode === Mode.Workflow && (
                          <React.Fragment>
                            ...
                            {workflowEnableUserConfigBuffers && (
                              <SettingRow className='mt-5' label={this.localeString('defaultSettings')} />
                            )}
                          </React.Fragment>
                        )}
                        <SettingRow>
              ...
              {workflowInfographicSidePopper && (
                <SidePopper isOpen title={widgetMode === Mode.Workflow ? this.localeString('customizeInfographicsBtn') : this.localeString('selectAnInfographic')} position='right' toggle={() => { this.toggleSidePopper('workflowInfographicSidePopper') }} trigger={this.sidePopperTrigger?.current}>
                  ...
                  <div className='p-3' style={popperOuterStyle}>
                    ...
                    <SettingRow className='mt-5 pb-2' label={this.localeString('selectInfographics')} />
          ...
          >
            <SettingRow flow='no-wrap' className='mt-3' label={this.localeString('viewMode')}>
            ...
            {displayHeader && (
              <React.Fragment>
                ...
                {stViewMode && stViewMode !== 'slides' && stViewMode !== 'auto' && (
          ...
          <SettingSection title={<div className='w-100 d-flex' style={{ height: '23px', flexFlow: 'column wrap', alignContent: 'space-between' }}>
            ...
            <UtilitySelector
              ...
              showRemove={false}
              closePopupOnSelect
              type={SupportedUtilityType.GeoEnrichment}
            />
          </SettingSection>
        </div>
      </div>

...
import TransportUtil from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/util/src/mobile/TransportUtil'
import Environments from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/util/src/Environments'
import Debounce from '../utils/debounce'
import GeocoderClient from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/util/src/mobile/GeocoderClient.js'
/**
 *  Map actions:
...
export default class BAMapActions {
  ...
  private initSearchControl () {
    ...
    if (self.showSearch) {
      ...
      self.searchControl = MapSearch.registerCallbacks(
        ...
        async function onMapSearchControlResults (results, bamInstance) {
  ...
  public renderLocation (data: any, preventZoom?: boolean) {
    ...
    if (this._initialized) {
      ...
      this.bufferType = data.bufferType as InfoBufferType
      this.bufferUnits = data.bufferUnits
      this.bufferSizes = data.bufferSizes
      this.bufferSizesString = JSON.stringify(this.bufferSizes).replace('[', '').replace(']', '')
    ...
    if (this.latitude && this.longitude && this.bufferType && this.bufferSizes && this.bufferUnits) {
      ...
      if (creds && creds.length === 2) {
        ...
        if (this.bufferType === 'drivetime') {
          resultObject = await TransportUtil.getBuffers('drivetime', sourceCountry, this.bufferUnits, this.bufferSizes, lon, lat)
   ...
   * ----------------------------------------------------------------------------------
   */
  private _renderToMap (noClear?: boolean, markerRenderFunc?: any, preventZoom?: boolean) {
    const self = this
    // this.syncSettings()
    if (!noClear) {
      // this.restartGraphics()
    ...
    if (self.context.presetBuffersHaveChanged) {
      self.context.presetBuffersHaveChanged = false
      renderBuffers = true
    }
    ...
    function _delay () {
      ...
      if (lat && lon) {
        ...
        // boundary - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        //
        const projectedRings = self.projectionCheck(geom)
  ...
  // Takes rings as input and may reproject to 102100 if needed
  projectionCheck (rings: any) {
    // rough check projection
    if (rings && rings.length > 0) {
      const coord = rings[0][0]
      if (coord[0] < -180 || coord[0] > 180 || coord[1] < -90 || coord[1] > 90) {
        ...
        rings.forEach(function (g) {
          const inner = []
          g.forEach((a) => {
            const c = BAProjection.pointToGeographic(a[0], a[1])
            inner.push([c.lon, c.lat])
          })
          newRings.push(inner)
        })
        return newRings

import {
  ...
  getAppStore, appActions, MutableStoreManager, type ExtentChangeMessage, type UseDataSource
} from 'jimu-core'
...
export default class MultiSourceMap extends React.PureComponent<Props, State> {
  ...
  onMapbaseExtentChanged = (dataSourceId: string, message: ExtentChangeMessage) => {
    ...
    if (currentVisibleDsId === dataSourceId) {
      ...
      if (this.state.firstMapDsId && this.state.firstMapDsId !== dataSourceId) {
        hiddenMapBase = this.refs.firstMapInstance as MapBase
      } else if (this.state.secondMapDsId && this.state.secondMapDsId !== dataSourceId) {
        hiddenMapBase = this.refs.secondMapInstance as MapBase
      }
      ...
      >
        <div ref={this.domRef} className='w-100 h-100 multi-source-map'>
          {!this.props.isDefaultMap && <div className='w-100 h-100 multi-map-container' style={{ position: 'relative' }}>
            ...
            >
              {this.state.firstMapDsId && this.props.isMapInVisibleArea && <MapBase
                ref='firstMapInstance'
                baseWidgetProps={this.props.baseWidgetProps} onMapLoaded={this.handleMapLoaded}
            ...
            >
              {this.state.secondMapDsId && this.props.isMapInVisibleArea && <MapBase
          ...
          {this.props.isDefaultMap && <div className='w-100 h-100 default-map-container' style={{ position: 'relative' }}>
            ...
            >
              { this.props.isMapInVisibleArea && <MapBase
          ...
          {this.state.currentJimuMapViewId && MapViewManager.getInstance().getJimuMapViewById(this.state.currentJimuMapViewId) &&
            <MapToolLayout
              ...
              LayoutEntry={this.props.baseWidgetProps.builderSupportModules && this.props.baseWidgetProps.builderSupportModules.LayoutEntry}
              layoutConfig={this.getLayoutConfig()} toolConfig={this.props.baseWidgetProps.config.toolConfig ? this.props.baseWidgetProps.config.toolConfig : {}}

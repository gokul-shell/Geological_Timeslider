/** @jsx jsx */
import { React, css, jsx, polished, classNames, ReactDOM, ErrorBoundary, ReactResizeDetector, type IntlShape } from 'jimu-core'
import { Icon, Popper, MobilePanelManager, type Modifiers } from 'jimu-ui'
    ...
    return (
      <div className='exbmap-ui exbmap-ui-tool' css={this._cssStyle()} style={{ width: '32px', height: '32px' }}>
        <div
          ...
          title={this.props.toolJson.isShowIconTitle ? this.getTitle() : ''} onClick={e => { this._onIconClick(e) }} aria-expanded={this.props.activeToolInfo?.activeToolName === this.toolName} role='button' tabIndex={0} onKeyDown={this._onKeyDown}
        >
        ...
        {this.iconContainer && (this.toolName === activeToolName && expandPanel) && <PanelShell onDestroyed={() => { this.onClosePanel() }}>
          <Popper
            ...
            modifiers={this.modifiers} version={this.generation} offset={[0, 4]} toggle={this.onPopperToggle} forceLatestFocusElements={true}
          >
              ...
              {({ mapWidgetHeight }) => (<div css={this._cssStyle()} className={this.getExpandPanelPlacementClassName()}>
                <div className='exbmap-ui-expand-content'>
                  <div className='w-100 justify-content-between d-flex exbmap-ui-expand-content-header'>
                    ...
                    <div onClick={() => { this.props.onActiveToolInfoChange(null) }} style={{ cursor: 'pointer' }} tabIndex={0} role="button" aria-label={this.props.intl.formatMessage({ id: 'close', defaultMessage: 'close' })}>

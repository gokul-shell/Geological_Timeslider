import {
  AbstractMessageAction, MessageType, type Message, type DataSourcesChangeMessage, DataSourcesChangeType, MutableStoreManager,
...
import { ADD_TO_MAP_DATA_ID_PREFIX, ActionType, type AddToMapDatas, DataChangeType, DataChangeStatus, MapViewManager } from 'jimu-arcgis'
...
export default class AddToMapAction extends AbstractMessageAction {
  ...
  onExecute (message: DataSourcesChangeMessage, actionConfig?: any): Promise<boolean> | boolean {
    ...
    message.dataSources.forEach(dataSourceParam => {
      ...
      const idBase = `${ADD_TO_MAP_DATA_ID_PREFIX}messageAction_${this.widgetId}_${dataSource.id}_`

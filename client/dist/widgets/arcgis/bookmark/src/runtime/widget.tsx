...
import { getBookmarkListFromRuntime, getKey, getOldKey } from './utils/utils'
...
> {
  ...
  onViewBookmark = (item: ImmutableObject<Bookmark>, isRuntime?: boolean, index?: number) => {
    ...
    if (!isRuntime && index === this.state.highLightIndex) {
      BookmarkNodeList.forEach(node => {
        if (node.classList.contains(currentBookmarkId)) {
          const currentBookmarkNodes = node.querySelectorAll('.bookmark-container .bookmark-pointer')
      ...
      if (jimuMapView && jimuMapView.view) {
        ...
        if (item?.environment && itemLighting) {
          ...
          view.environment.lighting = {
            ...
            directShadows: itemLighting.directShadows,
      ...
      ${'&.bookmark-widget-' + id} {
        ...
        .bookmark-container {
          ...
          .bookmark-pointer {
            cursor: pointer;
            &:hover {
              background: ${theme.colors.palette.light[200]} !important;
            }
          }
  ...
  renderSlideViewText = item => {
    ...
    return (
      <div
        className='w-100 h-100 bookmark-only-pointer surface-1 border-0'
        onClick={() => { this.onViewBookmark(item) }}
        key={item.id || `webmap-${item.name}`}
        role='button'
        tabIndex={0}
  ...
  renderCustomExample = () => {
    ...
    return (
      <div className='w-100 h-100 bookmark-only-pointer surface-1 border-0'>
        <LayoutEntry
          isRepeat
      ...
      switch (config.templateType) {
        case TemplateType.Card:
          runtimeBookmarkList = runtimeBookmarkArray.map((rbmId, index) => {
            const item = Immutable<Bookmark>(JSON.parse(utils.readLocalStorage(rbmId)))
            const titleTextInput = React.createRef<HTMLInputElement>()
            const isBookmarkItemActive = index === this.state.runtimeHighLightIndex
            return (
              <Fragment key={rbmId}>
                <div className='d-inline-flex bookmark-card-col runtime-bookmarkCard'>
                  <Card
                    onClick={() => { this.onViewBookmark(item, true, index) }}
                    className={classNames('surface-1 runtime-bookmark', { 'bookmark-pointer': appMode === AppMode.Run, 'active-bookmark-item': isBookmarkItemActive })}
                    button
                    tabIndex='0'
                    onKeyDown={evt => {
                      if (evt.key === 'Enter' || evt.key === ' ') {

...
import { Button, FloatingPanel, ImageFillMode, Popper } from 'jimu-ui'
import {
  type JimuMapView,
...
import { AppResourceManager, type ResourceItemInfo } from 'jimu-for-builder'
...
export class MarkPopper extends React.PureComponent<Props, States> {
  ...
  constructor (props) {
    ...
    this.popperRef = React.createRef<HTMLDivElement>()
  ...
  locateAndSetLayersConfig = (currentJimuMapView: JimuMapView) => {
    ...
    if (currentJimuMapView && currentJimuMapView.view) {
      ...
      const operationalLayers = (currentJimuMapView.view.map as any)?.resourceInfo?.operationalLayers || []
  ...
  focusOnCloseButton = () => {
    ...
    this.floatingPanelCloseButton?.focus()
  }
  ...
  handleClickClose = (evt, delLastFlag?: boolean) => {
    ...
    if (configUpdate && !delLastFlag) {
      ...
      this.addTemplateButtonRef.current.focus()
    }
  }

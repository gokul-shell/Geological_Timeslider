...
import {
  ...
  type IMThemeVariables, type ImmutableObject, type ImmutableArray, type BrowserSizeMode, LayoutType,
...
import {
  Checkbox, Icon, Button, defaultMessages as jimuUIDefaultMessages, TextInput, type ImageParam, TextArea,
  NumericInput, AdvancedButtonGroup, Select, Slider, ImageFillMode, Tabs, Tab, Tooltip, Switch, ConfirmDialog, CollapsablePanel
} from 'jimu-ui'
import { type IMConfig, TemplateType, type Bookmark, DirectionType, PageStyle, DisplayType, Status, ImgSourceType, type Transition } from '../config'
                      ...
                      </div>
                      {
                        ((!noImgType.includes(config.templateType)) && activeId === item.id) &&
                          <div className='active-mark-content'>
                            <div aria-label={this.formatMessage('imageSource')} role='group'>
                              <SettingRow label={this.formatMessage('imageSource')} className='mb-2'/>
                            ...
                            <SettingRow label={this.formatMessage('imagePosition')} className='mt-2' truncateLabel>
                              <div style={{ width: '40%' }}>
                                <Select size='sm' value={item.imagePosition} onChange={this.handleImageFillModeChange} aria-label={this.formatMessage('imagePosition')}>

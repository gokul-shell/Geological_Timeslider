...
import { type AnalysisToolParam } from 'analysis-ui-schema'
...
const CustomToolConfigPopperContent = (props: Props) => {
  ...
  const { toolInfo, output, option } = toolConfig.config as ImmutableObject<CustomToolConfig>
    ...
    <div css={style} onKeyDown={(e) => { e.stopPropagation() }}>
      ...
      <SettingSection>
        <CollapsablePanel className='config-section' label={translate('output')} type="default" defaultIsOpen={false} aria-label={translate('output')}>
          <CustomToolOutputConfig className='config-section-detail' parameters={toolInfo.parameters} output={output} onParameterChange={handleParameterChange} onOuputChange={handleOutputChange} />
        </CollapsablePanel>
      </SettingSection>
      <SettingSection>

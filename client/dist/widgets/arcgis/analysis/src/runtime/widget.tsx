...
import {
  React, jsx, type AllWidgetProps, loadArcGISJSAPIModules, urlUtils, css, uuidv1, Immutable, hooks, lodash, SessionManager, type IMState, ReactRedux, type ImmutableObject, requestUtils, loadArcGISJSAPIModule
} from 'jimu-core'
import { type IMConfig, type SubmissionData, type ToolConfig, ToolType, type StandardToolConfig, type HistoryItemWithDs, type StandardToolOption, type AnalysisGPJobSubmited, type CustomToolConfig } from '../config'
...
import { addLayerToMapByDs, createDsByResults, jobDidComplete, removeEmptyResults } from './utils'
...
import { deleteToolsHasNoMatchedUtility, destroyDataSources, filterHistoryItemMessages, useGetDisplayedToolName, wait } from '../utils/util'
...
import { cancelJob } from 'analysis-core'
import { getAppConfigAction } from 'jimu-for-builder'

const { useState, useMemo, useCallback, useEffect, useRef } = React

const useStyle = (isRTL: boolean) => {
  return useMemo(() => {
    return css`
      .jimu-tab {
        0.625rem;
...
const Widget = (props: AllWidgetProps<IMConfig>) => {
  ...
  useEffect(() => {
    defineCustomElements(window, { resourcesUrl: `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-app-assets/` })
    geoenrichmentComponentsDefineCustomElements(window, { resourcesUrl: `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-geoenrichment-components-assets/` })
    ...
    if (headElement) {
      ...
      linkElement.href = `${urlUtils.getAbsoluteRootUrl()}arcgis-amd-packages/arcgis-geoenrichment-components-assets/geoenrichment/themes/data-browser.css`
  ...
  const currentToolInfo = useMemo(() => {
    if (toolList.length === 1) {
      return toolList[0].asMutable({ deep: true })
    }
  ...
  }, [toolInfoStrings, toolList, translate])
  ...
  const handleJobStatusChange = useCallback(async (jobInfo: __esri.JobInfo, results?: __esri.ParameterValue[]) => {
    ...
    if (results) {
      ...
      if (toolInfo.type === ToolType.Standard && (toolInfo.config as StandardToolConfig).output.addResultLayersToMapAuto && dsMap) {
        dsMap.forEach((ds) => {
  ...
  const signIn = useCallback((toolInfo: ImmutableObject<ToolConfig>) => {
    if (toolInfo.type === ToolType.Custom) {
      const url = (toolInfo.config as ImmutableObject<CustomToolConfig>).toolUrl

      return requestUtils.requestWrapper(url, async (session) => {
        if (!esriRequestRef.current) {
          esriRequestRef.current = await loadArcGISJSAPIModule('esri/request')
        }
        return esriRequestRef.current(session?.token ? `${url}?token=${session.token}` : url, { query: { f: 'json' }, responseType: 'json' })
      })
    }
  ...
  const currentDisplayedToolProps = useMemo(() => {
    ...
    const presetMapHistory = currentToolInfo
      ? historyListFromMap.find((h) => {
        return currentToolInfo.type === ToolType.Standard && (currentToolInfo.config.option as StandardToolOption).presetFromMapHistoryId === h.id
      })
      : null
    return {
      theme,
      ...
      portal,
      toolInfo: Immutable(currentToolInfo || openedTool.toolInfo),
      ...
      translatedTitle: toolList.length === 1 ? getDisplayedToolName(currentToolInfo || openedTool.toolInfo) : undefined,
      onBack: onToolBack,
      signIn
    }
  }, [currentJimuMapView, currentToolInfo, getDisplayedToolName, historyListFromMap, onToolBack, openedTool, portal, signIn, theme, toolList.length, widgetConatiner, widgetId])

  const isRTL = ReactRedux.useSelector((state: IMState) => {
    return state.appContext.isRTL
  })

  const style = useStyle(isRTL)

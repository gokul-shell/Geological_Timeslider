...
import { getLocaleInfo, type LocaleItem, isTravelModeParameter, isEmptyDataItem, StraightLineParameterValue, formatNumberToLocale } from 'analysis-shared-utils'
import { type AnalysisToolData, type AnalysisParamModel, type AnalysisToolUITravelMode, type OutputName, AnalysisToolParamDataType, type AnalysisToolDataItem, type LayerUrlFilter, type FeatureCollection, type AnalysisToolContext, type AnalysisExtent, AnalysisEngine, type AnalysisRasterToolContext, AnalysisToolContextKeys, type AnalysisToolUI } from 'analysis-ui-schema'
...
const HistoryParameters = (props: Props) => {
  const { toolName, analysisEngine, jobParams, toolUiParameters, paramViewModel, toolT9n, toolUIJson, portal, type } = props
  ...
  const [envSettingContent, setEnvSettingContent] = useState(null)
  ...
  useEffect(() => {
    ...
    const { translatedKeys, translatedJobParams } = translateParameter(Immutable(combinedParameters).without('context').asMutable({ deep: true }))
  ...
  }, [parametersContainerNode, toolT9n])
  ...
  return (
    <React.Fragment>
      ...
      {type === ToolType.Standard && <CollapsablePanel label={translate('environmentSettings')} aria-label={translate('environmentSettings')} type="default" defaultIsOpen>
        <div css={style}>

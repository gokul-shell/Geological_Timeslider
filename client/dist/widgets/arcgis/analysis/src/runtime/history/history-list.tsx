...
import { CancelableStatus, UncancelableTools } from '../utils'
import { type AnalysisEngine, type AnalysisToolItem, UserPrivileges } from 'analysis-ui-schema'
import tools from '../../utils/tools.json'
import { hasNAPrivilege, hasGeoEnrichmentPrivilege, canPerformRasterAnalysis, canPerformStandardAnalysis, AnalysisJobStatus } from 'analysis-shared-utils'
...
const HistoryList = (props: Props) => {
  ...
  const [canPerformRasterAnalysisRes, setCanPerformRasterAnalysisRes] = useState(false)
  const [canPerformStandardAnalysisRes, setcanPerformStandardAnalysisRes] = useState(false)
  ...
  useEffect(() => {
    ...
    if (portal) {
      canPerformRasterAnalysis(portal).then((res) => { setCanPerformRasterAnalysisRes(res) }).catch((error) => {})
      try {
        setcanPerformStandardAnalysisRes(canPerformStandardAnalysis(portal))
      } catch (error) {}
    }
  ...
  const checkHistoryItemForPrivileges = (historyItem: HistoryItemWithDs): boolean => {
    ...
    const isRasterAnalysisEngineTool = historyItem.analysisEngine === 'raster'
    const rasterPrivilegesMet = isRasterAnalysisEngineTool ? canPerformRasterAnalysisRes : true
    privilegesMet.push(rasterPrivilegesMet)

    // Check for standard privileges
    const isStandardAnalysisEngineTool = historyItem.analysisEngine === 'standard'
    const standardPrivilegesMet = isStandardAnalysisEngineTool
      ? canPerformStandardAnalysisRes
      : true
  ...
  return historyList.length
    ? <List
        css={listStyle}
        itemsJson={historyList.map((history, index) => {
          const tool = toolList.find((t) => t.id === history.toolId)
          const isError = historyStatusIsError(history)
          const icon = isError ? require('jimu-icons/svg/outlined/suggested/wrong.svg') : getIconByToolName(history.toolName)
          return {
        ...
        renderOverrideItemContent={(actionData, refComponent) => {
          ...
          // TODO replace this with isStatusCancellable function in analysis after update version
          const jobStatus = history.jobInfo?.jobStatus
          const isStatusCancellable = CancelableStatus.includes(jobStatus)
          ...
          return <div className='progress-detail'>
            ...
            {(isStatusCancellable || isStatusCanceling) && <Loading className='running-loading' type="BAR" />}
          </div>
        }}
        renderOverrideItemCommands={(actionData, refComponent) => {
          ...
          const isToolCancellable = !(isSyncCustomTool || (tool?.type === ToolType.Standard && UncancelableTools.find((item: { toolName: string, analysisEngine: AnalysisEngine }) => {
          ...
          return <Dropdown onClick={(e) => { e.stopPropagation() }}>
            {!isStatusCancellable && <DropdownButton arrow={false} icon type='tertiary'>
      ...
      />
    : <div className='d-flex align-items-center justify-content-center' css={css`height: 100%; flex-direction: column; padding: 0 20%; color: var(--dark-500); text-align: center`}>

...
import BaseFlyController, { type prepareOptions, type flyOptions } from './base-fly-controller'
...
export default class RotatingFlyController extends BaseFlyController {
  ...
  _updateCamearGlByTilt = (tilt: number, opts?: { isUpdateCamearConfig: boolean }): void => {
    ...
    if (opts?.isUpdateCamearConfig) {
      const glCamera = new this.GLCamera({ eye: this._cache.cameraGL.pos, center: this._cache.lookAtTargetGL.pos, up: this._cache.cameraGL.upDir })

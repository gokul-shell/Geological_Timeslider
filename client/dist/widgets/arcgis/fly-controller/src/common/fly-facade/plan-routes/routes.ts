...
import { type FlyIds, type PrepareCmdOptions, type FlyCmdOptions, type StopCmdOptions } from '../fly-manager'
...
export default class Routes {
  ...
  async _getFlyRecordDynamically (prepareCmdOpts: PrepareCmdOptions, sceneView: __esri.SceneView, callbacks: FlyStateChangeCallbacks): Promise<Record> {
    ...
    const record = await new Record().setup(recordConfig, sceneView, callbacks, prepareCmdOpts.justForHightlight)
    return record
  }
  ...
  _getLimitionParamForPrepare (record: Record, prepareCmdOpts: PrepareCmdOptions): prepareOptions {
    ...
    if (type === ControllerMode.AroundMapCenter && (this.itemConfig as RotateItemConfig).targetMode === RotateTargetMode.MapCenter) {
      ...
      Object.assign(limition, { aroundMapCenterConfig: _aroundMapCenterConfig })
    }
  ...
  async highlightSpecifiedRecord (): Promise<any> {
  ...
  async prepareSpecifiedRecord (prepareCmdOpts: PrepareCmdOptions, flyCallbacks: FlyStateChangeCallbacks): Promise<{ isCanPlay: boolean, prepareState: boolean }> {
    ...
    if (prepareCmdOpts.justForHightlight) {

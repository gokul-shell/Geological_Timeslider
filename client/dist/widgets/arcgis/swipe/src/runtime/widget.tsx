...
const Widget = (props: AllWidgetProps<IMConfig>) => {
  ...
  useEffect(() => {
    ...
    //After layer added, get the runtime added layer.
    const addLayerListener = () => {
    ...
    //Listen to add layer created event.
    activeMapView?.addJimuLayerViewCreatedListener(addLayerListener)

    //Watch remove layer event, reset the runtime added layer.
    const handler: IHandle = activeMapView?.view?.map?.layers?.on('after-remove', (event) => {
      const runtimeLayerList = getRuntimeAddedJimuLayerViews(activeMapView)
      setRuntimeAddedActiveMapViewJimuLayerViews(Immutable(runtimeLayerList))
      setIsRuntimeSwipeLayerUpdated(true)
      setIsRuntimeScrollLayerUpdated(true)
    ...
    return () => {
      activeMapView?.removeJimuLayerViewCreatedListener(addLayerListener)
      handler?.remove()
    }

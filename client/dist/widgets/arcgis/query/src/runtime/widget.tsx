...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig>> {
  ...
  render () {
    ...
    if (config.arrangeType === QueryArrangeType.Popper) {
      return (
        <QueryWidgetContext.Provider value={`${layoutId}:${layoutItemId}`}>
          ...
          >
            <QueryTaskList widgetId={id} isInPopper queryItems={config.queryItems} defaultPageSize={config.defaultPageSize} className='py-3' />
          </TaskListPopperWrapper>
    ...
    return (
      <div
        className='jimu-widget runtime-query py-3 surface-1 border-0'
        css={widgetStyle}
      >
        <QueryWidgetContext.Provider value={`${layoutId}:${layoutItemId}`}>

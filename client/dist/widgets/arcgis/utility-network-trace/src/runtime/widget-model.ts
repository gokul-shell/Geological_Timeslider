import { DataSourceManager, MessageManager, DataRecordsSelectionChangeMessage, type DataSourceJson, type ImmutableObject, type FeatureLayerQueryParams } from 'jimu-core'
import { DataSourceStatus, type FeatureLayerDataSource } from 'jimu-core/data-source'
    ...
    this.unt.on('select-features', (res) => {
      ...
      Promise.all(fetchRecordPromises).then((fetchedRecords) => {
        ...
        if (allRecords.length > 0) {
          ...
          const message = new DataRecordsSelectionChangeMessage(this.props.id, allRecords)

...
> {
  ...
  getInitConfig = () => {
    ...
    if (!this.props.config.messageUseDataSource) {
      if (
        ...
        const dsJson =
          config.dataSources[messageWidgetJson.useDataSources[0].dataSourceId]
        if (
          dsJson &&
          ...
          messageUseDataSource = Immutable({
            ...
            mainDataSourceId:
              messageWidgetJson.useDataSources[0].mainDataSourceId,
            dataViewId: messageWidgetJson.useDataSources[0].dataViewId,
            rootDataSourceId:
              messageWidgetJson.useDataSources[0].rootDataSourceId
          })
        }
      }
    } else {
      messageUseDataSource = this.checkAndGetInitUseDataSource(
        messageWidgetId,
        this.props.config.messageUseDataSource
      )
    }
    ...
    if (oldActionUseDataSource) {
      if (actionUseDataSource) {
        if (
          oldActionUseDataSource.dataSourceId ===
    ...
    if (isMapDs) {
      ...
      if (widgetJson && widgetJson.useDataSources) {
        for (let i = 0; i < widgetJson.useDataSources.length; i++) {
          if (
            widgetJson.useDataSources[i].dataSourceId ===
  ...
  checkIsShowSetData = (widgetId: string) => {
    ...
    if (
      ...
      const dsJson =
        config.dataSources[widgetJson.useDataSources[0].dataSourceId]
      if (
        dsJson &&
        (dsJson.type === ArcGISDataSourceTypes.WebMap ||
          dsJson.type === ArcGISDataSourceTypes.WebScene)
  ...
  _getDsIdsByWidgetId = (wId: string) => {
    ...
    return Immutable(
      widgetJson?.useDataSources?.map(
        (useDS: ImmutableObject<UseDataSource>) => useDS.dataSourceId
      ) ?? []
    )
  }
  ...
  _getDsRootIdsByWidgetId = (wId: string) => {
    ...
    widgetJson?.useDataSources?.forEach(
      (useDS: ImmutableObject<UseDataSource>) => {
        const ds = dsM.getDataSource(useDS.dataSourceId)
        if (
          ds?.type === DataSourceTypes.WebMap ||
          ds?.type === DataSourceTypes.WebScene
        ) {
          // is root ds
  ...
  checkTrigerLayerIsSameToActionLayer = () => {
    if (
      ...
      if (
        this.props.config.messageUseDataSource.dataSourceId ===
  ...
  render () {
    ...
    const actionUseDataSourceInstance =
      this.props.config.actionUseDataSource &&
        ...
        {((!!config.messageUseDataSource && !!config.actionUseDataSource) ||
          ...
          <Fragment>
            <SettingSection title={this.formatMessage('conditions')}>
              {isExtendChange && (
                <SettingRow
                  label={this.formatMessage('queryWithCurrentExtent')}
                >
                  <Switch
                    checked={config.enableQueryWithCurrentExtent}
                    onChange={evt => {
                      this.swicthEnabledQueryWithCurrentExtent(
                        evt.target.checked
                      )
                    }}
                  />
              ...
              {!isExtendChange && (
                <SettingRow label={this.formatMessage('relateMessage')}>
                  <Switch
                    checked={config.enabledDataRelationShip}
                    onChange={evt => {
              ...
              {!isExtendChange && this.props.config.enabledDataRelationShip && (
                <SettingRow flow='wrap'>
                  {this.checkTrigerLayerIsSameToActionLayer() && (
                    <div className='w-100 border p-1 mr-2'>
                      {this.formatMessage('autoBind')}
                    </div>
                  )}
                  {!this.checkTrigerLayerIsSameToActionLayer() && (
                    <div className='w-100 d-flex align-items-center'>
                      <div className='d-flex flex-column relate-panel-left'>
                        <FieldSelector
                          ...
                          useDataSources={Immutable([
                            this.props.config.messageUseDataSource?.asMutable({
                              deep: true
                            })
                          ])}
                        ...
                        <FieldSelector
                          ...
                          useDataSources={Immutable([
                            this.props.config.actionUseDataSource?.asMutable({
                              deep: true
                            })
                          ])}
                          isDataSourceDropDownHidden
                          onChange={this.onActionFieldSelected}
                          useDropdown
                          isSearchInputHidden
                          selectedFields={
                            this.props.config.actionUseDataSource &&
                      ...
                      <Icon
                        ...
                        icon={require('jimu-ui/lib/icons/link-combined.svg')}
                      />
                    </div>
                  )}
                </SettingRow>
              ...
              <SettingRow
                label={
                  <Button
                    className='p-0'
                    type='link'
                    disabled={!this.props.config.actionUseDataSource}
                    onClick={this.showSqlExprPopup}
                  >
                    {this.formatMessage('moreConditions')}
                  </Button>
                }
              />
              <SettingRow>
                {this.props.config.actionUseDataSource && (
                  <DataSourceComponent
                    useDataSource={this.props.config.actionUseDataSource}
                  >
                    {ds => {
                      return (
                        <SqlExpressionBuilderPopup
                          ...
                          expression={this.props.config.sqlExprObj}
                          onChange={sqlExprObj => {
                ...
                <div className='sql-expr-display body-1'>
                  {this.props.config.sqlExprObj && actionUseDataSourceInstance
                    ? dataSourceUtils.getArcGISSQL(
                      this.props.config.sqlExprObj,
                      actionUseDataSourceInstance
                    ).displaySQL

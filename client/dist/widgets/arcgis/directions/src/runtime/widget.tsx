/** @jsx jsx */
import { React, jsx, css, type AllWidgetProps, DataSourceManager, DataSourceStatus, type FeatureLayerDataSource, dataSourceUtils, type UseUtility, UtilityManager, getAppStore, hooks, MutableStoreManager } from 'jimu-core'
...
const Widget = (props: AllWidgetProps<IMConfig>) => {
  ...
  const isDarkTheme = props.theme?.darkTheme
  ...
  const getAddressFromSources = useCallback(async (point: __esri.Point) => {
    const sourcesPromise = searchConfig.dataConfig.map(async c => {
      return await getUrlOfUseUtility(c.useUtility)
    })
    const DEFAULT_GEOCODING_URL = 'https://geocode-api.arcgis.com/arcgis/rest/services/World/GeocodeServer'
    const sources = await Promise.all(sourcesPromise)
    sources.push(DEFAULT_GEOCODING_URL)
  ...
  const updateFromDataAction = useCallback(async () => {
    if (isReadyToRenderDirections() && directionsRef.current && props.mutableStateProps) {
      const { directionsFromPoint, directionsToPoint } = props.mutableStateProps
  ...
  }, [getAddressFromSources, isReadyToRenderDirections, props.mutableStateProps, props.widgetId])
  ...
  useEffect(() => {
    ...
    async function updateDirectionsWidget () {
      ...
      const newRouteLayer = new RouteLayer({ id, url: routeServiceUrl })
  ...
  }, [id, jimuMapView?.view, searchConfig, routeConfig?.useUtility, defaultSearchHint, isReadyToRenderDirections, isDarkTheme, props.autoHeight, updateFromDataAction])
...
async function createJSAPILayerForDs (ds: FeatureLayerDataSource, geoType: 'point' | 'polyline', source: __esri.Graphic[]) {
  ...
  const fields = dataSourceUtils.changeJimuFieldsToJSAPIFields(ds.getSchema().fields, idField)
  await ds.updateSourceByFeatures(source, fields, idField?.name, {
    id: ds.id,

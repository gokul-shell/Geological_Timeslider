...
/** @jsx jsx */
import { React, type AllWidgetProps, BaseWidget, jsx, classNames, getAppStore, WidgetState, AppMode, type IMState, type DataRecord, DataSourceManager, DataSourceStatus, type FeatureLayerQueryParams, type QueriableDataSource, type DataSource, geometryUtils } from 'jimu-core'
import {
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
  ...
  getSelectedFeatureRecords = (currentDs: string) => {
    ...
    this.waitForChildDataSourcesReady(this.mapView).finally(() => {
        ...
        setTimeout(() => {
          if (ds.getSelectedRecords().length > 0) {
            ...
            if (selectedFeatureRecord?.[0].feature) {
              const dsLayerId = this.getDSLayerID(selectedFeatureRecord?.[0].feature.layer.id)
            ...
            if (isFeatureValid) {
              ...
              this.displayFeaturesResult([[selectedFeatureRecord?.[0]?.feature]])
            }
          }
        }, 500)
      })
  ...
  activeViewChangeHandler = async (jmv: JimuMapView) => {
    ...
    this.waitForChildDataSourcesReady(jmv).finally(() => {
      this.setState({
      ...
      if (jmv) {
        this.setState({
          ...
          setTimeout(() => {
            ...
            this.getSelectedFeatureRecords(this.state.jimuMapView.dataSourceId)
          }, 100)
        })
      }
    })
  ...
  */
  ...
  getFeatureFromLayer = async (layerObject, oid) => {
    const query = new Query()
    query.where = layerObject.objectIdField + ' = ' + oid
    query.outFields = ['*']
    query.returnGeometry = true
    query.returnZ = true
    query.outSpatialReference = this.state.jimuMapView?.view?.spatialReference
    return layerObject.queryFeatures(query).then(featureSet => {
      return featureSet?.features[0] ?? null
    })
  }

  displayFeaturesResult = (featuresByLayer) => {
    this.setState({
      initialStage: false,
      resultStage: true,
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (currentWidgetState === WidgetState.Opened || !currentWidgetState) {
      ...
      if (featureRecords?.length > 0 &&
        ...
        if (validRecord) {
          this.getFeatureFromLayer(validRecord.dataSource.layer, validRecord.getId()).then((feature) => {
            this.displayFeaturesResult([[feature]])
          })
        } else {
          return
        }
    ...
    if (prevProps.useMapWidgetIds !== this.props.useMapWidgetIds ||
      ...
      if (this.props.config.configInfo[this.props.config.activeDataSource]) {
        this.setState({
          ...
          this.checkLineLayerAvailableInDsAndConfig(this.state.currentDatasource)
          this.getSelectedFeatureRecords(this.state.currentDatasource)
          this.setState({
  ...
  buildStatsValuesAsOutput = (profileResult, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip) => {
    ...
    //define dummy point geometry as for profile stats value we don't have any geometry
  ...
  checkForIntersectingLayer = (skipSettingResultState: boolean | undefined): Promise<any[]> => {
    return new Promise((resolve) => {
      ...
      if (this.intersectingLayersAtRuntime?.length > 0) {
        ...
        const supportedLayers = ['group', 'feature']
        //get the layers from map of type Group and Feature also filter out the layers which are not visible
        const layersArray = this.mapView.view.map.layers.toArray().filter((layer) => {
          return supportedLayers.includes(layer.type) && this.considerLayerVisibility(layer)
        })
        const assetLayersCurrentConfig = this.props.config.configInfo[this.state.currentDatasource]?.assetSettings?.layers
        dataSource?.forEach((layer) => {
          this.intersectingLayersAtRuntime.forEach((intersectingLayerId) => {
            if (intersectingLayerId === layer.id) {
              layersArray.forEach((viewLayers: any) => {
                if (viewLayers.type === 'group') {
                  const grpLayer = viewLayers.layers.toArray()
                  grpLayer.forEach((grpLayers) => {
                    //check if the id matches and the layer is visible or not
                    if ((grpLayers.id === layer.jimuChildId) && this.considerLayerVisibility(grpLayers)) {
                      const currentSetting = assetLayersCurrentConfig.find(configLayer => intersectingLayerId === configLayer.layerId)
                      defArr.push(this.queryForIntersectingLayers(currentSetting, selectableLineGeom))
                    }
                  })
                } else if (viewLayers.type === 'feature') {
  ...
  queryForIntersectingLayers = async (assetLayerSettings, selectableLineGeom?): Promise<LayerIntersectionInfo | null> => {
    ...
    return new Promise((resolve) => {
      if (ds?.layerDefinition?.geometryType === 'esriGeometryPolyline' || ds?.layerDefinition?.geometryType === 'esriGeometryPoint') {
        ...
        intersectingFeatureQuery.outFields = ['*']
  ...
  */
  ...
  queryForIndividualLayers = (dsId: string, geometry) => {
    const defArr = []
    const dataSource: DataSource[] = getAllLayersFromDataSource(dsId)
    ...
    let isGroupLayerVisible: boolean = true
    ...
    this.canShowProfilingForBackward(this.props.config.configInfo[this.state.currentDatasource]))) {
      dataSource?.forEach((layer) => {
        ...
        this.selectableLayersAtRuntime.forEach((selectableLayerId) => {
          if (selectableLayerId === eachLayer.id) {
            layersArray.forEach((viewLayers) => {
              const layersArray: any = viewLayers
              if (layersArray.type === 'group') {
                isGroupLayerVisible = this.considerLayerVisibility(layersArray)
                if (isGroupLayerVisible) {
                  const grpLayer = layersArray.layers.toArray()
                  grpLayer.forEach((grpLayers) => {
                    isLayerVisible = this.considerLayerVisibility(grpLayers)
                    if ((grpLayers.id === eachLayer.jimuChildId) && isLayerVisible) {
                      if (eachLayer.layerDefinition?.geometryType && eachLayer.layerDefinition.geometryType === 'esriGeometryPolyline') {
                        defArr.push(this.queryIndividualLayer(grpLayers, eachLayer, geometry))
                      }
                    }
                  })
                }
    ...
    return (
      <div css={getStyle(this.props.theme)} className='jimu-widget'>
        <div className='widget-elevation-profile'>
          {this.state.onWidgetLoadShowLoadingIndicator &&

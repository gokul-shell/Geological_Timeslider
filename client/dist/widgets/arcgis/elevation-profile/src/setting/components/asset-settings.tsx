...
import { ArcGISDataSourceTypes, type JimuMapView, loadArcGISJSAPIModules, MapViewManager } from 'jimu-arcgis'
...
import { type AssetSettings, type AssetLayersSettings } from '../../config'
...
import { defaultSelectedUnits, getMaxBufferLimit, getPortalSelfElevationUnits, getRandomHexColor, validateMaxBufferDistance } from '../../common/utils'
...
export default class AssetSetting extends React.PureComponent<Props, IState> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      layerName: null,
      bufferDistanceInput: this.props.config.assetIntersectingBuffer?.bufferDistance ? this.props.config.assetIntersectingBuffer?.bufferDistance : 10,
      bufferUnit: configuredBufferDistanceUnit,
      bufferSymbol: null,
  ...
  componentDidMount = () => {
    if (!this.state.apiLoaded) {
      loadArcGISJSAPIModules([
        ...
        this.setState({
          apiLoaded: true,
          bufferSymbol: this.props.config.assetIntersectingBuffer?.bufferSymbol ? this.jsonUtils.fromJSON(this.props.config.assetIntersectingBuffer?.bufferSymbol) : this.jsonUtils.fromJSON(defaultBufferSymbol)
        }, () => {
          this.props.onAssetBufferSettingsUpdated('bufferSymbol', this.state.bufferSymbol?.toJSON())
        })
      })
    }
  ...
  */
  ...
  showLayerData = (item: any, index?: number) => {
    ...
    const dsObj = DataSourceManager.getInstance().getDataSource(item.layer.id)
    ...
    //get the layer data from map object
    const individualLayerData = this.mapView.view.map.findLayerById(dsObj.jimuChildId)
    const mapLayer: any = individualLayerData
    //check the availability of vertical unit parameter in case of z value
    this.hasSupportForZValue = mapLayer.hasZ
  ...
  */
  ...
  onBufferUnitsChange = (evt: any) => {
    const bufferDistanceMaxLimit = validateMaxBufferDistance(this.state.bufferDistanceInput, evt.target.value)
    this.props.onAssetBufferSettingsUpdated('bufferDistance', bufferDistanceMaxLimit)
    this.setState({
      bufferUnit: evt.target.value,
      bufferDistanceInput: bufferDistanceMaxLimit
    }, () => {
      setTimeout(() => {
  ...
  onBufferSymbolChanged = (currentSymbol) => {
    ...
    if (currentSymbol === null) {
      symbol = this.jsonUtils.fromJSON(defaultBufferSymbol)
    }
    this.setState({
      bufferSymbol: symbol
    }, () => {
        ...
        if (subLayers) {
          subLayers.forEach(subLayer => {
            if (subLayer.layerDefinition?.geometryType === 'esriGeometryPolyline' ||
              subLayer.layerDefinition?.geometryType === 'esriGeometryPoint') {
    ...
    return <div css={getAdvanceSettingsStyle(this.props.theme)} style={{ height: '100%', width: '100%', marginTop: 5 }}>
      ...
      {this.props.config.assetIntersectingBuffer?.enabled &&
        <div className={'pt-2 pb-2'} aria-expanded={this.props.config.assetIntersectingBuffer?.enabled}>
          <SettingRow>
            <NumericInput style={{ width: 55 }} aria-label={this.state.bufferUnit} title={this.state.bufferDistanceInput + ''}
              size={'sm'} defaultValue={this.state.bufferDistanceInput} min={0} max={getMaxBufferLimit(this.state.bufferUnit)}
              value={this.state.bufferDistanceInput} showHandlers={false} onChange={this.onBufferDistanceChange} />

            <Select menuRole={'menu'} aria-label={this.state.bufferUnit} style={{ width: 141 }} className={'pl-2 pr-2'}
            ...
            {this.state.bufferSymbol &&
              <SymbolSelector
                jimuSymbolType={JimuSymbolType.Polygon}
                symbol={this.state.bufferSymbol}

...
import { React, jsx, type IMThemeVariables, type IntlShape, classNames, getAppStore } from 'jimu-core'
...
import { type ProfileChart, type Statistics } from '../../config'
import { unitOptions, getConfigIcon } from '../constants'
import StatisticsList from './common-statistics-list'
import { loadArcGISJSAPIModules } from 'jimu-arcgis'
import { getPortalSelfElevationUnits, getPortalSelfLinearUnits } from '../../common/utils'
...
interface IState {
  ...
  apiLoaded: boolean
  isActiveWebscene: boolean
  volumetricObjectsLabel: string
}
...
export default class ProfileChartSettings extends React.PureComponent<Props, IState> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      isActiveWebscene: isWebscene,
      volumetricObjectsLabel: this.props.config.volumetricObjLabel ? this.props.config.volumetricObjLabel : this.nls('volumetricObjectsLabel')
    }
  }
  ...
  onVolumetricObjLineColorChange = (color: string) => {
    this.props.onProfileChartSettingsUpdated('volumetricObjLineColor', color)
  }

  onshowVolumetricObjLineInGraphChange = (evt: any) => {
    this.props.onProfileChartSettingsUpdated('showVolumetricObjLineInGraph', evt.target.checked)
  }

  onVolumetricObjLabelChange = (e) => {
    const value = e.target.value
    this.setState({
      volumetricObjectsLabel: value
    })
  }

  onVolumetricObjLabelAcceptValue = (value) => {
    this.setState({
      volumetricObjectsLabel: value.trim()
    }, () => {
      this.props.onProfileChartSettingsUpdated('volumetricObjLabel', this.state.volumetricObjectsLabel)
    })
  }
    ...
    return <div style={{ height: '100%', width: '100%', marginTop: 5 }} css={getProfileChartStyle(this.props.theme)}>
      <div>
        ...
        <SettingRow>
          <div className='d-flex justify-content-between w-100 align-items-center'>
            <div className='align-items-center d-flex'>
              <Radio role={'radio'} data-testid={'ground'} disabled={this.state.isGroundDisabled}
        ...
        <SettingRow>
          <div className='d-flex justify-content-between w-100 align-items-center'>
            <div className='align-items-center d-flex'>
              <Radio role={'radio'} data-testid={'custom'}
        ...
        {this.state.isActiveWebscene &&
          <React.Fragment>
            <SettingRow className={'pt-3 ep-divider-top'}>
              <Label tabIndex={0} aria-label={this.nls('volumetricObjectsLabel')} className='w-100 d-flex'>
                <div className='flex-grow-1 text-break color-label setting-text-level-1'>
                  {this.nls('volumetricObjectsLabel')}
                </div>
              </Label>
            ...
            <SettingRow label={this.nls('showInGraph')}>
              <Switch role={'switch'} aria-label={this.nls('showInGraph')}
                title={this.nls('showInGraph')}
                checked={this.props.config.showVolumetricObjLineInGraph}
                onChange={this.onshowVolumetricObjLineInGraphChange} />
            </SettingRow>

            {this.props.config.showVolumetricObjLineInGraph &&
              <React.Fragment>
                <SettingRow label={this.nls('viewLineColor')}>
                  <ColorPicker aria-label={this.nls('viewLineColor')} title={this.nls('viewLineColor')} placement={'top'}
                    offset={[0, 0]} showArrow color={this.props.config.volumetricObjLineColor ? this.props.config.volumetricObjLineColor : '#b54900'}
                    onChange={this.onVolumetricObjLineColorChange} />
                </SettingRow>

                <SettingRow label={this.nls('label')} flow={'wrap'}>
                  <TextInput role={'textbox'} aria-label={this.nls('volumetricObjectsLabel')} size={'sm'} className={'w-100'}
                    value={this.state.volumetricObjectsLabel} onAcceptValue={this.onVolumetricObjLabelAcceptValue} onChange={this.onVolumetricObjLabelChange} />
                </SettingRow>
              </React.Fragment>
            }
          </React.Fragment>
        ...
        <SettingRow className={'pt-3 ep-divider-top'} label={this.nls('displayGroundProfileStatsLabel')}>

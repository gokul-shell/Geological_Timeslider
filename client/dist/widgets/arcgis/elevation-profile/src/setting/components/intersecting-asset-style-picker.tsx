  ...
  render () {
    return <div className={'pt-4 pb-3'}>
      <SettingRow>
        ...
        <Select menuRole={'menu'} aria-label={this.nls('styleShape') + this.state.intersectingAssetShape} size={'sm'} name={'symbolPicker'}
          ...
          {this.props.layerGeometryType === 'esriGeometryPoint' && intersectingAssetShapeList.map((item, index) => {
            ...
            return <Option role={'option'} aria-label={item} key={index} value={item.value} title={this.nls(item.label)}>
        ...
        <NumericInput aria-label={this.nls('styleSize')} size={'sm'} className={'ml-2'} min={1} max={20}
          defaultValue={this.state.intersectingAssetSize} onChange={this.onIntersectingAssetSizeChange} />
      </ SettingRow>
    </div>
  }
}

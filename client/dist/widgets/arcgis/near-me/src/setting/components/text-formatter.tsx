...
interface Props {
  ...
  onTextFormatChange: (textMessage: string, textStyle: FontStyleSettings) => void
  onTextMessageChange: (textMessage: string, textValue: string) => void
}
...
export default class TextFormatSetting extends React.PureComponent<Props, State> {
  ...
  onTextChange = (textValue: string) => {
    this.setState({
      promptMessage: textValue
    })
    this.props.noResultFound
      ? this.props.onTextMessageChange(this.props.noResultFound, textValue)
      : this.props.onTextMessageChange(this.props.promptTextMessage, textValue)
  }
  ...
  textStyleObj = (fontFamily: FontFamilyValue, fontBold: boolean, fontItalic: boolean, fontUnderline: boolean, fontStrike: boolean,
    ...
    this.props.noResultFound
      ? this.props.onTextFormatChange(this.props.noResultFound, textStyle)
      : this.props.onTextFormatChange(this.props.promptTextMessage, textStyle)
  }
    ...
    return (
      <div style={{ height: '100%', width: '100%' }}>
        <SettingRow flow={'wrap'}>
          <TextArea tabIndex={0} className='w-100' name='text' aria-label={this.state.promptMessage}
              defaultValue={this.props.hintMessage}

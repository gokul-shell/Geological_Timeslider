...
export default class AoiTool extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (prevProps.config?.distanceUnits !== this.props.config?.distanceUnits) {
      this.setState({
        defaultDistanceUnits: this.props.config?.distanceUnits ? this.props.config?.distanceUnits : this.state.defaultDistanceUnits
      })
    }
    ...
    //check if show sketch tools and distance changed
    if (prevProps.config?.showDistanceSettings !== this.props.config?.showDistanceSettings ||
      prevProps.config?.showSketchTools !== this.props.config?.showSketchTools) {
      this.setState({
        showDistSettings: this.props.config?.showDistanceSettings,
        showSketchTools: this.props.config?.showSketchTools
      }, () => {
  ...
  render () {
    ...
    const bufferToolStyles = this.props.widgetWidth < 306 || (!this.state.searchByActiveMapArea && !this.state.showDistSettings) || (!this.props.headingLabel && this.state.showDistSettings && !this.state.showSketchTools)
    ...
    return (
      ...
      <div className='pt-2 px-2' css={getAoiToolStyle(this.props?.theme, this.state.showSketchTools)}>

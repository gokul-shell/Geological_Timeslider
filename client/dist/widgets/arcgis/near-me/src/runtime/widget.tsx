...
/** @jsx jsx */
import { type JimuMapView, JimuMapViewComponent, geometryUtils, type FeatureLayerDataSource } from 'jimu-arcgis'
import {
...
import { type IconComponentProps, Loading, LoadingType, WidgetPlaceholder, utils, Alert, Label, Button, defaultMessages as jimuUIDefaultMessages } from 'jimu-ui'
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, State> {
      ...
      if (!lodash.isDeepEqual(prevActiveDsConfig?.searchSettings, currentActiveDsConfig?.searchSettings)) {
        if (this.didSearchSettingsChanged(prevActiveDsConfig?.searchSettings, currentActiveDsConfig?.searchSettings)) {
          ...
          this.setState({
            ...
            if (currentActiveDsConfig?.searchSettings?.onlyShowLayersResult !== prevActiveDsConfig?.searchSettings?.onlyShowLayersResult) {
              this.onOnlyShowLayerResultsChanged()
            }
            // check if show distance label is changed
            if (currentActiveDsConfig?.searchSettings?.showDistFromInputLocation !== prevActiveDsConfig?.searchSettings?.showDistFromInputLocation) {
  ...
  geometriesByDsIdFromAction = (selectedGeometriesByDsId) => {
    ...
    this.actionTimeout = setTimeout(async () => {
      ...
      dsIds.forEach((dsId) => {
        const geometryType = this.geometriesFromAction[dsId][0].type
  ...
  */
   ...
   */
  createFeatureSet = (featureList: DataRecord[], layerInfo: LayersInfo, objIdField: string, distanceUnit: __esri.LinearUnits, analysisId: string) => {
        ...
        group.features.forEach((feature, featureIndex: number) => {
          featureItems.push(
            <FeatureSet
              ...
              showDistFromInputLocation={this.state.searchSettings.showDistFromInputLocation}
              isGroup={true}
   ...
   */
  performAnalysis = async (layerInfo, analysisId: string) => {
      ...
      if (this.featuresByDsId.hasOwnProperty(dsId)) {
        ...
        if (featureList.length > 0) {
          if (layerInfo.analysisInfo.analysisType === AnalysisTypeName.Summary) {
            ...
            featureSet = this.createFeatureSet(featureList, layerInfo, objIdField, distanceUnit as __esri.LinearUnits, analysisId)
            resolve(featureSet)
          }
        } else {
          resolve(featureSet)
      ...
      dsManager.createDataSource(Immutable({
        ...
        actionsPromise.then(async actions => {
          ...
          if (action?.length > 0) {
            ...
            await DataActionManager.getInstance().executeDataAction(firstAction, [dataSets], DataLevel.Records, this.props.widgetId)
          }
        }).catch(err => {
          console.error(err)
        })
      ...
      this.state.analysisSettings?.layersInfo.forEach((layerInfo, index) => {
        ...
        if (!((showAllFeatures || searchCurrentExtent) && layerInfo.analysisInfo.analysisType === AnalysisTypeName.Closest)) {
          ...
          if (this.availableLayersIds.includes(layerInfo.useDataSource.dataSourceId)) {
            ...
            defArray.push(this.performAnalysis(layerInfo, analysisId))
          }
        }
      })
      Promise.all(defArray).then((results: any) => {
        results.forEach((result, index: number) => {
          if (result?.count > 0) {
            ...
            if (this.state.searchSettings?.onlyShowLayersResult) {
  ...
  filterToOnlyShowResultFeatures = (dsId, records?) => {
    if (this.state.searchSettings?.onlyShowLayersResult && dsId && this.featuresByDsId[dsId]) {
      const ds = getSelectedLayerInstance(dsId) as any
      if (ds) {
  ...
  resetFilters = (forceReset?: boolean) => {
    ...
    if ((this.state.searchSettings?.onlyShowLayersResult || forceReset) && this.state.jimuMapView) {
 ...
 */
  ...
  render () {
    ...
    const showLoadingIndicator = !this.state?.jimuMapView ||
    ...
    return (
      <div css={getStyle(this.props.theme, this.state.listMaxHeight, this.state.generalSettings.noResultMsgStyleSettings, this.state.generalSettings.promptTextMsgStyleSettings)} className={'jimu-widget'}>
              ...
              */}
              {(this.state.displayLayerAccordion?.length > 0 ||
                (this.state.displayLayerAccordion.length === 0 && this.state.showNoResultsFoundMsg && (showAllFeatures || (searchByLocation && this.state.aoiGeometries)) &&
                  this.state.analysisSettings?.layersInfo.length > 0 && this.state.isAnalysisLayerConfigured)) &&
